...
### Pengujian distirbusi kartesian
...
### Pengujian distribusi graph

Salah satu pengujian yang dapat dilakukan pada topologi graph adalah memperoleh informasi dimensi. Hal ini dapat dilakukan dengan memanfaatkan ``MPI_Graphdims_get()`` dan ``MPI_Graph_get()``.
...
| ------------- |:-------------:|
|comm|input communicator|
|nnodes|output jumlah node|
|nedges|otuput jumlah edge|
|maxindex|panjang vektor index|
|maxedges|panjang vektor edge|
|index|output array yang berisi struktur graph|
|edges|output array yang berisi struktur graph untuk edge|
